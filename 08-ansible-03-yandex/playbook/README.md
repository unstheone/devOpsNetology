<br>tested on Centos7, but can be adopted for other distributions
<br>*don't forget to enter your host IP address in ``vars.yml``*
---

[site.yml](site.yml) содержит 3 блока: Clickhouse, Vector & Lighthouse

```inventory: ```

| Hostname          | Public IP       | Software           |
|-------------------|-----------------|--------------------|
| clickhouse-01     | 158.160.116.255 | Clickhouse         |
| vector-01         | 51.250.86.234   | Vector             |
| lighthouse-01     | 158.160.105.206 | Lighthouse & Nginx |

## Clickhouse
### Объединяет последовательность задач по инсталяции Clickhouse. Блоку соответствует ```tags: clickhouse```
#### Параметры по умолчанию
- ```clickhouse_version: "22.4.3.3"``` - версия Clickhouse
...
## LightHouse
### Объединяет последовательность задач по инсталяции Vector. Блоку соответствует ```tags: lighthouse```
#### Параметры по умолчанию
- ```lighthouse_code_src: "https://github.com/VKCOM/lighthouse.git"``` - исходный код lighthouse
- ```lighthouse_code_src_version: d701335``` - версия кода
- ```lighthouse_data_dir: "/home/user/lighthouse/"``` - домашная директория

#### Последовательность Task'ов:
- ```[Lighthouse. Pre-install epel-release]``` - устанавливает pre-req для последущей установки ngninx
- ```[Lighthouse. Nginx & Git client]``` - устанавливает nginx & git
- ```[Lighthouse. Clone Lighthouse source code by Git]``` - скачивает исходных код lighthouse
- ```[Lighthouse. Prepare nginx config]``` - настраивает nginx & lighthouse
